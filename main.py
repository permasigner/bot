...
watching_statuses = ["for /", f"Python version {platform.python_version()}", "app signing requests", "GitHub actions",
                     "Nebula develop"]
...
@bot.event
async def on_ready():
    logger.info("Permacord Bot")
    logger.info(
        f'Logged in as: {bot.user.name} - {bot.user.id} ({discord.__version__})')
    logger.info(f'Ready to serve the server!')
    ...
    await bot.loop.create_task(Bot.change_statuses())
