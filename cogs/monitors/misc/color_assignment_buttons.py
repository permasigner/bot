...
from utils.views import ColorRoleButton


class ColorAssignButtons(commands.Cog):
    ...
    async def on_ready(self):
        ...
        for _, mapping in rero_mappings.items():
            ...
            for emoji, role in mapping.items():
                ...
                view.add_item(ColorRoleButton(role, emoji))

            self.bot.add_view(view)

    buttons = app_commands.Group(
        name="colors", description="Interact with color buttons", guild_ids=[cfg.guild_id])

    @admin_and_up()
    @buttons.command(description="Post the button color assignment message")
    @transform_context
    async def post_message(self, ctx: GIRContext):
        # timeout is None because we want this view to be persistent
        channel = ctx.guild.get_channel(
            guild_service.get_guild().channel_reaction_colors)
        if channel is None:
            raise commands.BadArgument("Color assignment channel not found!")

        embed = discord.Embed(
            description="Click the buttons to choose a color of your choice!", color=discord.Color.blurple())
        await channel.send(embed=embed)
        await ctx.send_success(f"Posted in {channel.mention}!")

    @admin_and_up()
    @buttons.command(name="set", description="Prompt to add color assignment buttons to a message")
    ...
    async def _set(self, ctx: GIRContext, message_id: str):
        ...
        async with ctx.channel.typing():
            for r in reaction_mapping[message.id]:
                ...
                view.add_item(ColorRoleButton(
                    ctx.guild.get_role(reaction_mapping[message.id][r]), r))
            await message.edit(view=view)

        await ctx.send_success(title="Reaction colors set!", description=resulting_reactions_list)

    @admin_and_up()
    @buttons.command(description="Add a new color assignment button to a message")
    ...
    async def prompt_for_reaction(self, ctx: GIRContext, reactions):
        ...
        await ctx.send_success(description=text, title="Reaction colors")
        ...
        async with ctx.channel.typing():
            for m in rero_mapping:
                ...
                for r in rero_mapping[m]:
                    ...
                    view.add_item(ColorRoleButton(role, r))
                await message.edit(view=view)
...
async def setup(bot):
    await bot.add_cog(ColorAssignButtons(bot))
