...
class Memes(commands.Cog):
    def __init__(self, bot):
        ...
        self.meme_phrases = ["{user}, have a look at this funny meme! LOL!", "Hey, {user}. Have a look at this knee-slapper!", "{user} 😂😂😂😂😭😭😭😭"]
        self.snipe_cache = {}
    ...
    async def meme(self, ctx: GIRContext, name: str, user_to_mention: discord.Member = None):
        ...
        if bucket.update_rate_limit(current) and not (gatekeeper.has(ctx.guild, ctx.author, 5) in ctx.author.roles):
