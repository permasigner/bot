...
support_tags = [tag.name for tag in guild_service.get_guild().tags if "support" in tag.name]
...
async def handle_support_tag(ctx: GIRContext, member: discord.Member) -> None:
    ...
    if bucket.update_rate_limit(current) and not (gatekeeper.has(ctx.guild, ctx.author, 6) or ctx.guild.get_role(guild_service.get_guild().role_moderator) in ctx.author.roles):
