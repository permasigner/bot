...
class Filters(commands.Cog):
    ...
    @transform_context
    async def add(self, ctx: GIRContext, bypass: int, phrase: str) -> None:
        fw = FilterWord()
        fw.bypass = bypass
        fw.word = phrase
        ...
        await ctx.send_success(title="Added new word to filter!", description=f"This filter can be bypassed by level {bypass}, and the phrase is `{phrase}`")
